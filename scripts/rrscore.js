...
function kidneyrisk(lang) {
    ...
    if (sum_risk > 0) {
        ...
        if (lang == 'th') {
            ...
            if (tc[7] < 60 && tc[5] > 0) {
                res = 'ท่านอาจมีโรคไตอยู่แล้วในปัจจุบัน';
                sug = 'ควรปรึกษาแพทย์';           
            } else {                
                //result
                if (sum_risk < 0.05) {
                    res = 'ท่านมีความเสี่ยง <em>น้อย</em> ต่อการเกิดโรคไตในระยะเวลา 10 ปี <em>(น้อยกว่า 5%)</em>';
                } else if (sum_risk <= 0.1) {
                    res = 'ท่านมีความเสี่ยง <em>ปานกลาง</em> ต่อการเกิดโรคไตในระยะเวลา 10 ปี <em>(' + Math.round(sum_risk * 100) + '%)</em>';
                } else if (sum_risk <= 0.15) {
                    res = 'ท่านมีความเสี่ยง <em>สูง</em> ต่อการเกิดโรคไตในระยะเวลา 10 ปี <em>(' + Math.round(sum_risk * 100) + '%)</em>';
                    perc = ' และควรตรวจสุขภาพกับแพทย์อย่างสม่ำเสมอ';
                } else if (sum_risk <= 0.2) {
                    res = 'ท่านมีความเสี่ยง <em>สูงกว่าปกติ</em> ต่อการเกิดโรคไตในระยะเวลา 10 ปี <em>(' + Math.round(sum_risk * 100) + '%)</em>';
                    perc = ' และควรตรวจสุขภาพกับแพทย์อย่างสม่ำเสมอ';
                } else if (sum_risk > 0.2) {
                    res = 'ท่านมีความเสี่ยง <em>สูงมาก</em> ต่อการเกิดโรคไตในระยะเวลา 10 ปี <em>(มากกว่า 20%)</em>';
            ...
            if (tc[7] < 60 && tc[5] > 0) {
                res = 'You may have kidney disease already.';
                sug = 'Please consult your physician';
            } else {
                if (sum_risk < 0.05) {
                    res = 'Your 10-Year Kideney Disease Risk is <em>low</em>. <em>(less than 5%)</em>';
                } else if (sum_risk <= 0.1) {
                    res = 'Your 10-Year Kideney Disease Risk is <em>average</em>. <em>(' + Math.round(sum_risk * 100) + '%)</em>';
                } else if (sum_risk <= 0.15) {
                    res = 'Your 10-Year Kideney Disease Risk is <em>high</em>. <em>(' + Math.round(sum_risk * 100) + '%)</em>';
                    perc = ' and annual health checkup';
                } else if (sum_risk <= 0.2) {
                    res = 'Your 10-Year Kideney Disease Risk is <em>too high</em>. <em>(' + Math.round(sum_risk * 100) + '%)</em>';
                    perc = ' and annual health checkup';
                } else if (sum_risk > 0.2) {
                    res = 'Your 10-Year Kideney Disease Risk is <em>very high</em>. <em>(over 20%)</em>';
